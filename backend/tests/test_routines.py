...
async def test_list_routines_isolated_per_user(client: AsyncClient):
    ...
    await client.post("/api/routines", json={"title": "U1", "schedule_type": "daily"}, headers=h1)
    await client.post("/api/routines", json={"title": "U2", "schedule_type": "daily"}, headers=h2)

    r1 = await client.get("/api/routines", headers=h1)
    r2 = await client.get("/api/routines", headers=h2)
...
async def test_update_routine(client: AsyncClient):
    ...
    created = await client.post("/api/routines", json={"title": "Old", "schedule_type": "daily"}, headers=headers)
    rid = created.json()["id"]
    resp = await client.patch(f"/api/routines/{rid}", json={"title": "New", "is_paused": True}, headers=headers)
...
async def test_delete_routine(client: AsyncClient):
    ...
    created = await client.post("/api/routines", json={"title": "Del", "schedule_type": "daily"}, headers=headers)
    rid = created.json()["id"]
    resp = await client.delete(f"/api/routines/{rid}", headers=headers)
    assert resp.status_code == 204
    after = await client.get("/api/routines", headers=headers)
...
async def test_goal_routine_link_lifecycle(client: AsyncClient):
    ...
    goal = await client.post("/api/tasks", json={"title": "Health"}, headers=headers)
    goal_id = goal.json()["id"]
    routine = await client.post("/api/routines", json={"title": "Workout", "schedule_type": "daily"}, headers=headers)
    rid = routine.json()["id"]

    link = await client.post("/api/routines/links", json={
        "goal_id": goal_id,
        "routine_id": rid,
    ...
    listing = await client.get(f"/api/routines/links/by-goal/{goal_id}", headers=headers)
    ...
    dup = await client.post("/api/routines/links", json={
        "goal_id": goal_id, "routine_id": rid, "start_date": "2026-01-01",
    }, headers=headers)
    assert dup.status_code == 409

    # Delete link
    rm = await client.delete(f"/api/routines/links/{link_id}", headers=headers)
