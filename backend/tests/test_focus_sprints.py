...
async def test_create_focus_sprint(client: AsyncClient):
    ...
    resp = await client.post("/api/focus-sprints", json={
        "title": "Q1 sprint",
...
async def test_focus_sprint_isolated_per_user(client: AsyncClient):
    ...
    await client.post("/api/focus-sprints", json={
        "title": "U1", "start_date": "2026-01-01", "end_date": "2026-01-31",
    }, headers=h1)
    r1 = await client.get("/api/focus-sprints", headers=h1)
    r2 = await client.get("/api/focus-sprints", headers=h2)
...
async def test_add_and_remove_items(client: AsyncClient):
    ...
    goal = await client.post("/api/tasks", json={"title": "Goal X"}, headers=headers)
    goal_id = goal.json()["id"]
    routine = await client.post("/api/routines", json={"title": "Daily X", "schedule_type": "daily"}, headers=headers)
    routine_id = routine.json()["id"]

    sp = await client.post("/api/focus-sprints", json={
        "title": "Mix", "start_date": "2026-01-01", "end_date": "2026-01-31",
    }, headers=headers)
    sp_id = sp.json()["id"]

    # Add a goal and a routine — verify hydration batch returns titles
    r1 = await client.post(f"/api/focus-sprints/{sp_id}/items", json={
        "item_type": "goal", "goal_id": goal_id,
    }, headers=headers)
    r2 = await client.post(f"/api/focus-sprints/{sp_id}/items", json={
    ...
    fetched = await client.get("/api/focus-sprints", headers=headers)
    ...
    # Remove one
    item_id = items[0]["id"]
    rm = await client.delete(f"/api/focus-sprints/{sp_id}/items/{item_id}", headers=headers)
...
async def test_add_item_validates_ownership(client: AsyncClient):
    ...
    other_goal = await client.post("/api/tasks", json={"title": "Other"}, headers=h1)
    ...
    resp = await client.post(f"/api/focus-sprints/{sp_id}/items", json={
